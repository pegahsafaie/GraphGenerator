...
public class PersonalityRecognizer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (cmd.hasOption("m")) {
				recognizer.setModel(Integer.parseInt(cmd.getOptionValue("m")) - 1);
			}
	...
	public static double[] extractPersonality(int modelIndex, boolean selfModel, String text){
		try {
			...
			recognizer.setModel(modelIndex-1);

			// load models for all Big Five traits
			Classifier[] models = recognizer.loadWekaModels(selfModel, false);
			Map<String, Double> counts = recognizer.getFeatureExtractionFromText(text, true);
			double[] scores = recognizer.runWekaModels(models, counts);
			return scores;
		}
	...
	private void loadProperties(File propFile) {
		...
		try {
			...
			attributeFile = new File("/home/pegah/IdeaProjects/PersonalityExtraction/lib/attributes-info.arff");
